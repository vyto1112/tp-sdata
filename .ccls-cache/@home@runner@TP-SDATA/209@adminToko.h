...
//Prosedur untuk memisahkan data
void tokenize20(string s, string del = " ", int idx =0)
{
	string tmp;
	int i;
  int start;
...
void AdminT::pohon()
{
  ...
  else if (pil == 5)
  {
    ...
    for(int i = 0; i < 7; i++)
    {
      if(ubah == pt[i].pegawai)
      {
        cout << "Data ditemukan";
        cout << endl;
        cout << "Masukkan nama data yang baru : ";
        cin >> baru;
...
void AdminT::menuantrian(){  
  ambildata();
  for (int i = 0; i < idx2-1; i++)
  {
    enqueueLinkedListAT(qAT[i].nama, qAT[i].NIK);
  }
